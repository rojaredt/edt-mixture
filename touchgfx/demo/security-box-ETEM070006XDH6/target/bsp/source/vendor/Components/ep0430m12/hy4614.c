...
static uint32_t conversion_ratio_width = 0;
static uint32_t conversion_ratio_height = 0;
...
void conversion_ratio_Cal(void)
{
    ...
    agv_x = TARGET_WIDTH << 12;
    tmp = 0;
    times = 0;
    
    for(i = 0; i < agv_x; i++)
    {
        tmp = tmp + SUM_WIDTH_TEMP;
        times++;
        if(tmp > agv_x)
            break;
    }
    conversion_ratio_width = times - 1;
    
    agv_y = TARGET_HEIGHT << 12;
    tmp = 0;
    times = 0;
    
    for(i = 0; i < agv_y; i++)
    {
        tmp = tmp + SUM_HEIGHT_TEMP;
        times++;
        if(tmp > agv_y)
            break;
    }
    conversion_ratio_height = times - 1;
}
...
{
  conversion_ratio_width = conversion_ratio_height;
  conversion_ratio_height = conversion_ratio_width;
  
  #ifdef XY_CONVERSION
      conversion_ratio_Cal();
  #endif
...
void hy4614_TS_GetXY(uint16_t DeviceAddr, uint16_t *X, uint16_t *Y)
{
  ...
  if(hy4614_handle.currActiveTouchIdx < hy4614_handle.currActiveTouchNb)
  {
    ...
    #ifdef XY_CONVERSION
        *X = (coord * conversion_ratio_width ) >> 12;
    #else
        *X = coord;
    #endif
    ...
    #ifdef XY_CONVERSION
        *Y = (coord * conversion_ratio_height ) >> 12;
    #else
        *Y = coord;
    #endif    
